...
**Keywords:** river water temperature; retrospective hindcasting; physics-inspired
...
This study asks a narrow empirical question: on fixed common station/date/horizon
...
that heat budget ([Caissie, 2006](https://doi.org/10.1111/j.1365-2427.2006.01597.x)).
Earlier approaches range from nonlinear air–water regressions
([Mohseni et al., 1998](https://doi.org/10.1029/98WR01877)) to hybrid air-temperature
and discharge models ([Toffolon and Piccolroaz, 2015](https://doi.org/10.1088/1748-9326/10/11/114011))
and broader statistical comparisons ([Piccolroaz et al., 2016](https://doi.org/10.1002/hyp.10913)).
...
[Zwart et al., 2023](https://doi.org/10.3389/frwa.2023.1184992)). ThermoRoute's
...
wind ([USGS NWIS](https://doi.org/10.5066/F7P55KJN);
[Daymet V4](https://doi.org/10.3334/ORNLDAAC/2129);
[Abatzoglou, 2013](https://doi.org/10.1002/joc.3413)). LightGBM is the principal
tree baseline ([Ke et al., 2017](https://proceedings.neurips.cc/paper/2017/hash/6449f44a102fde848669bdd9eb6b76fa-Abstract.html)),
...
The canonical file is `data_usgs/panel_usgs_120v2.parquet`, bound to
`data_usgs/station_registry_v1.csv` by `data_usgs/frozen_panel_v1.json`.
...
temperature (`WTEMP`), streamflow (`FLOW`), air temperature (`TEMP`), precipitation
(`PRCP`), a relative-humidity proxy (`RHMEAN`), Daymet daylight-period mean
incoming shortwave radiation in W m⁻² (`DH`), and wind speed (`WDSP`). `DH` is not
day length, and `RHMEAN` is derived from temperature and vapor pressure rather than
...
2.8% for `FLOW`, about 0.07% for `TEMP`, `PRCP`, `RHMEAN`, and `DH`, and zero for
`WDSP`. These rates describe the committed derived panel; they do not reconstruct
...
Let `y_t` be the last observed daily water temperature and `c_{t+h}` a frozen
seasonal climatology. Damped persistence moves `y_t` toward `c_{t+h}` at a fixed
...
mixtures ([Shazeer et al., 2017](https://arxiv.org/abs/1701.06538)).

### 3.3 Bounded residual and probabilistic heads

The learned point correction is transformed by `tanh` and scaled by a fixed
`delta_scale`, limiting its numerical deviation from the anchor. This yields a
...
horizon, q05 and q95 are clipped to the nominal raw q50 if needed, and q50 is
...
unbounded residual. Each Stage 09 control uses the same five seeds (0--4) as
ThermoRoute and is paired within seed on identical forecast keys and exact target
bytes before ensemble averaging. These are deletion/intervention sensitivities;
they do not prove component necessity, identify a causal mechanism, or provide a
capacity-matched attribution. Every comparison uses identical
station/date/horizon target keys for the two models.

The separate Stage 09b PlainMLP and PlainCausalTCN comparisons are prospectively
information-matched as well as parameter-matched. Both controls receive the same
outcome-free issue-time history, station identity, climatology, frozen damped
anchor, standardized environmental context, and calendar/regime-gate tensors
available to full ThermoRoute. They predict unrestricted residuals around the
same damped anchor, but omit the learned relaxation proposal, lag router, mixture
of experts, and residual bound. They never read the target tensor, target-date
field, or disabled water-level channel. This removes a direct information-set
confound, but it does not equalize historical hyperparameter-search budgets or
make the comparison causal.

The original sealed protocol called the exploratory controls single-seed. An
independently sealed, outcome-free model-matrix amendment prospectively supersedes
that exploratory replication rule with five fixed seeds for every Stage 09
control and the exact 45-member Stage 09b matrix. The amendment does not change
the five confirmatory comparisons, their estimands or margins, the data splits,
primary models, CQR/Platt contracts, or any claim decision rule. Earlier
single-seed and 31-member control artifacts are invalid for the amended design.
...
but still consumes each target site's observed WTEMP history through the issue
...
rather than clipping them or calling them drought; PRCP uses nonnegative `log1p`.
...
The current canonical configuration sets `delta_scale` to 1.0 °C. It is a
...
perturbations. They cannot promote or replace a failed formal test. Stage 18 records
`REV_NOT_EVALUATED_NO_PREDECLARED_COST_LOSS_RATIOS`: no numerical relative economic
value is computed because no cost--loss ratios, real management costs, losses,
actions, or stakeholder utility model were frozen. Even a future preregistered
cost--loss grid would remain a hypothetical development sensitivity without those
observed decision quantities.
...
A separate temporal-calibration sensitivity groups consecutive positions in the
retained, issue-date-sorted calibration rows. Its block size of seven therefore
means seven retained rows, not seven calendar days. The delayed-ACI sensitivity
uses each forecast's target date as an idealized feedback-arrival proxy; the inputs
contain no verified observation-publication timestamp, revision history, data
vintage, or reporting latency. It neither replays real feedback availability nor
establishes conditional coverage; it reports empirical marginal coverage only.
...
exists, a network continuation may fetch only ledger entries without a durably
published canonical transaction, under the same opening identifier, and only while
normalized, derived, and trusted outputs are absent. HTTP retries are allowed; a
response received before its transaction directory is complete and durable may be
requested again, whereas a complete, durable, verifiable canonical response is
never replaced. A partial, invalid, or noncanonical canonical transaction fails
closed without overwrite; cleanup is limited to unpublished owner-private
temporary or pending state. Once all raw transactions validate, the request map,
two normalized tables, and manifest are
generated and validated in one private same-filesystem stage and published by one
directory rename. After the manifest exists, raw-network continuation and relaunch
of the raw child are prohibited, but network-free deterministic completion under
...
generation without a receipt is fully replayed before receipt creation. An
abandoned stage is deleted only after its canonical name, owner-private mode,
same-device read-only regular files, and absence of external hard links all
validate; any unsafe stage fails closed. Partial, invalid, or noncanonical
canonical contents fail closed and are not replaced. A valid receipt missing only
its external digest sidecar may regenerate that sidecar
...
pre-label protocol, the outcome-free inference-scope and model-matrix amendments,
deterministic model serialization contracts, development replay
...
1. rerun the canonical six-model development experiment, the seven mandatory
   same-five-seed diagnostic controls, and the separate matched
   information-and-parameter-controlled plain-neural/five-seed feature-ladder audit;
