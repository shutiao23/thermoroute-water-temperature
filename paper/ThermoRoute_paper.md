...
κ   = σ(β_s + c_q·z(logFLOW) + c_ℓ·z(WLEVEL) + w·season)  (daily relaxation rate)
...
All code, fixed split boundaries, per-day predictions, the USGS acquisition pipeline
(public-domain NWIS + Daymet), unit tests, and one-command reproduction are in this
repository. An adversarial internal review (`scripts`-driven) checked every headline
claim against the result tables; negative results and ablations are retained.
