...
\section{Pre-opening results placeholder and stopping rule}\label{results-placeholder-and-stopping-rule}

No formal result block is present in this byte-frozen pre-opening snapshot. After
a valid receipt exists, \texttt{python scripts/\allowbreak{}26\_\allowbreak{}validate\_\allowbreak{}claims.py --root . --registry protocols/\allowbreak{}route\_a\_claim\_registry\_v1.json --write-generated-results --require-complete}
derives five deterministic blocks from the exact statistics artifact and atomically
and idempotently writes the canonical Markdown evidence layer. Each must appear
exactly once, including negative, conflicting, or non-estimable outcomes.
Handwritten substitutions are rejected by body hashes and evidence bindings. This
TeX file remains a pre-opening methods snapshot and is not automatically made
submission-ready; a separately verified publication generator is required after
Route-A review.
...
authorization, and release gate passes. Any later receipt-derived Markdown result
layer supersedes only this pre-opening readiness status; the old headline numbers
