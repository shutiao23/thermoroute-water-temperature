...
arm here retains the target gauge\textquotesingle s own observed history, so nothing in this
...
case (\citeA{siddik2026conus};
\citeA{philippus2026ungaged}) and
in networks far sparser than this cohort
\cite{chang2025arctic}. We measure how
much a benchmark\textquotesingle s construction moves a reported number; we do not measure
...
development-evaluation partition the registry holds 249,072 station/\allowbreak date/\allowbreak horizon
...
gage height (00065) come from USGS NWIS as the provider\textquotesingle s 00003 daily-mean
statistic on site-local dates. Discharge is served in ft³ s⁻¹ and \textbf{is consumed
...
\textbf{Three evidence grades are used, and they are not interchangeable.} Section
4.4 is \emph{confirmatory}: its model suite, five-test family, estimand, margins, and
decision rules were fixed on data through 2020 and the 2021--2023 window was
opened only to score them. Sections 4.1--4.3 are \emph{development diagnostics} on the
...
development keys. Sections 4.5--4.10 are \emph{post-outcome descriptive} analyses,
...
stations on the relevant common key registry (\texttt{outputs/\allowbreak{}final/\allowbreak{}}), in °C. Pooled
...
ThermoRoute reaches 1.694 °C against the tree\textquotesingle s 1.735 °C. Against the
information-matched plain convolutional network the paired station median is
−0.015 °C at seven days in ThermoRoute\textquotesingle s favour, on the sign convention of
equation (2); this contrast is outside the sealed family and carries no
decision.

\textbf{Each test is read at the margin it was sealed at.} The five-test family is
...
\emph{non-inferiority} tests at a +0.05 °C margin, a strict numerical ceiling on
allowable degradation fixed on 2026-07-22 under an attestation that no post-2020
outcome had been requested or inspected
(\texttt{protocols/\allowbreak{}route\_\allowbreak{}a\_\allowbreak{}confirmatory\_\allowbreak{}v1.json}, sealed by
\texttt{route\_\allowbreak{}a\_\allowbreak{}protocol\_\allowbreak{}seal\_\allowbreak{}v1.json}). The margin is a number, not a judgement: the
protocol\textquotesingle s own rationale records that it is not derived from sensor precision,
...
sealed decision rule --- Holm-adjusted one-sided p ≤ 0.05 \emph{and} the whole
percentile interval below the registered margin.

Two limits on that statement. Row 4 is a \emph{supported non-inferiority} result at a
...
ThermoRoute\textquotesingle s favour but with an interval covering zero, so that row establishes
...
\textbf{Table 2 --- the five sealed tests, each at its registered margin.} ThermoRoute
...
intervals; sign-flip p-values computed against the row\textquotesingle s own margin and
...
components are in \texttt{outputs/\allowbreak{}final/\allowbreak{}sealed\_\allowbreak{}confirmatory\_\allowbreak{}family\_\allowbreak{}v1/\allowbreak{}}. The margin
...
\caption{\textbf{Spatial transfer under matched random-site and whole-region holdouts on the independent 2021--2023 window.} (a) The four whole-HUC2 folds. Panels (b)
and (c) show \textbf{ThermoRoute} under the earlier region-transfer run
(\texttt{outputs/\allowbreak{}conventional/\allowbreak{}region\_\allowbreak{}transfer\_\allowbreak{}metrics\_\allowbreak{}2021\_\allowbreak{}2023.csv}), \emph{not} the
...
qualifies a station on its \emph{all-keys} record; inside a stratum the same station
...
same size and should not be described with the same word: pooling a station\textquotesingle s
...
\textbf{Table 3 --- value of realized future meteorology.} Station-first forcing
...
\subsection{Future weather is worth less, not more, when local observations are withheld}\label{future-weather-is-worth-less-not-more-when-local-observations-are-withheld}
...
This analysis is post-outcome and descriptive, and is quoted in the Key Points under that label. It is the study\textquotesingle s most interpretable descriptive result and its least tested one --- the kind of finding that ought to be pre-registered before it is believed (\texttt{outputs/\allowbreak{}final/\allowbreak{}forcing\_\allowbreak{}information\_\allowbreak{}interaction\_\allowbreak{}v1/\allowbreak{}}).

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\subsection{The estimator matters only where the information does not}\label{the-estimator-matters-only-where-the-information-does-not}
...
\textbf{In the regime this literature reports, the two implementations differ by about
...
contrast supports \emph{the estimator not being where this problem\textquotesingle s difficulty
...
equivalence. Section 4.4\textquotesingle s two LightGBM rows are the only negligibility
...
cluster-bootstrap interval except the last. \emph{Reference construction} is the
