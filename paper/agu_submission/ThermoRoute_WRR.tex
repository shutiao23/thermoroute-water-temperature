...
% Result-placeholder macro for the held-out 2021--2023 metric cells.  Renders as
% a clearly marked <<...>> token in typewriter; the empty braces break the << / >>
% guillemet ligature under T1 so the angle brackets stay literal, and
% \textless/\textgreater keep them correct under OT1 as well.  Underscores in the
% argument must be escaped (\_).
...
on a held-out 2021--2023 test window (Section 4.6); those comparative metrics
are reported as clearly marked placeholders pending computation. Reported skill
for this variable is therefore largely a statement about the reference model and
the spatial partition rather than about the architecture.
...
(\href{https://www.jstor.org/stable/4615733}{Holm, 1979}) over these five
...
should be read as descriptive sensitivities, not as decision
evidence. We do not treat a failure to reject as evidence of equivalence, and no
comparison here is written as a statement of superiority, non-inferiority,
equivalence, or parity, or as a generalization to a national or U.S.-river
population. Equal-HUC aggregation, leave-one-HUC2 influence, and a year-by-season
temporal-stability audit (equal weighting of the 12 year-by-season cells,
leave-one-year, and leave-one-season for DJF, MAM, JJA, SON) are reported as
sensitivities on the held-out window. HUC2 is a coarse administrative grouping
and is not an independent river-network component.
...
placeholders (\ph{\dots}) pending computation from
\texttt{outputs/\allowbreak{}conventional/\allowbreak{}holdout\_\allowbreak{}metrics\_\allowbreak{}2021\_\allowbreak{}2023.csv}.
...
2023-12-31. Metric cells below are clearly marked placeholders (\ph{\dots})
pending computation; a parallel computation writes the long-form table
\texttt{outputs/\allowbreak{}conventional/\allowbreak{}holdout\_\allowbreak{}metrics\_\allowbreak{}2021\_\allowbreak{}2023.csv}
(columns \texttt{model},\allowbreak \texttt{horizon},\allowbreak \texttt{metric},\allowbreak \texttt{value},\allowbreak \texttt{n}), from which these
cells are filled. No value here is invented.

\textbf{Table 4.6 --- paired comparisons on the held-out window.} Median station
ΔRMSE (equation 9, °C, negative favours ThermoRoute), the ThermoRoute win rate,
and the approximate whole-HUC2 cluster-bootstrap 95\% interval, for each row of
the Section 3.6 comparison set. Codes: DP = damped persistence, LG = LightGBM.

{\footnotesize
\setlength{\tabcolsep}{3pt}
\renewcommand{\arraystretch}{1.12}
\noindent
\begin{tabularx}{\dimexpr\linewidth-6pt\relax}{@{}l>{\raggedright\arraybackslash}Xr>{\centering\arraybackslash}X>{\centering\arraybackslash}X>{\centering\arraybackslash}X>{\centering\arraybackslash}X@{}}
\toprule\noalign{}
\# & Comparison & Lead & ΔRMSE (°C) & CI low & CI high & Win rate \\
\midrule\noalign{}
1 & ThermoRoute vs. damped persistence & 1 d & \ph{dRMSE\_DP\_h1} & \ph{CIlo\_DP\_h1} & \ph{CIhi\_DP\_h1} & \ph{win\_DP\_h1} \\
2 & ThermoRoute vs. damped persistence & 3 d & \ph{dRMSE\_DP\_h3} & \ph{CIlo\_DP\_h3} & \ph{CIhi\_DP\_h3} & \ph{win\_DP\_h3} \\
3 & ThermoRoute vs. damped persistence & 7 d & \ph{dRMSE\_DP\_h7} & \ph{CIlo\_DP\_h7} & \ph{CIhi\_DP\_h7} & \ph{win\_DP\_h7} \\
4 & ThermoRoute vs. LightGBM & 3 d & \ph{dRMSE\_LG\_h3} & \ph{CIlo\_LG\_h3} & \ph{CIhi\_LG\_h3} & \ph{win\_LG\_h3} \\
5 & ThermoRoute vs. LightGBM & 7 d & \ph{dRMSE\_LG\_h7} & \ph{CIlo\_LG\_h7} & \ph{CIhi\_LG\_h7} & \ph{win\_LG\_h7} \\
\bottomrule\noalign{}
\end{tabularx}
}

\textbf{Tables 4.7--4.8 --- held-out 2021--2023 metrics.} Station-median metrics
per model and lead on the common test keys: RMSE, MAE, and bias in °C; skill
against persistence and against climatology (equation 10, dimensionless,
positive favours the candidate); and the reportable station count \emph{n}. Table 4.7
lists the six primary models and Table 4.8 the six one-factor ablations of
Section 3.2, on the same test keys and denominators; the ablations are deletion
and intervention sensitivities and do not prove component necessity. Both tables
are set in landscape.
...
\centering
\textbf{Table 4.7.} Held-out 2021--2023 station-median metrics, primary models.\\[3pt]
\begin{tabular}{@{}lrcccccc@{}}
\toprule\noalign{}
Model & $h$ (d) & RMSE ($^\circ$C) & MAE ($^\circ$C) & bias ($^\circ$C) & skill vs.\ persist & skill vs.\ clim & $n$ \\
\midrule\noalign{}
Persistence & 1 & \ph{RMSE\_Persistence\_h1} & \ph{MAE\_Persistence\_h1} & \ph{bias\_Persistence\_h1} & \ph{skill\_vs\_persistence\_Persistence\_h1} & \ph{skill\_vs\_climatology\_Persistence\_h1} & \ph{n\_Persistence\_h1} \\
Persistence & 3 & \ph{RMSE\_Persistence\_h3} & \ph{MAE\_Persistence\_h3} & \ph{bias\_Persistence\_h3} & \ph{skill\_vs\_persistence\_Persistence\_h3} & \ph{skill\_vs\_climatology\_Persistence\_h3} & \ph{n\_Persistence\_h3} \\
Persistence & 7 & \ph{RMSE\_Persistence\_h7} & \ph{MAE\_Persistence\_h7} & \ph{bias\_Persistence\_h7} & \ph{skill\_vs\_persistence\_Persistence\_h7} & \ph{skill\_vs\_climatology\_Persistence\_h7} & \ph{n\_Persistence\_h7} \\
DampedPersistence & 1 & \ph{RMSE\_DampedPersistence\_h1} & \ph{MAE\_DampedPersistence\_h1} & \ph{bias\_DampedPersistence\_h1} & \ph{skill\_vs\_persistence\_DampedPersistence\_h1} & \ph{skill\_vs\_climatology\_DampedPersistence\_h1} & \ph{n\_DampedPersistence\_h1} \\
DampedPersistence & 3 & \ph{RMSE\_DampedPersistence\_h3} & \ph{MAE\_DampedPersistence\_h3} & \ph{bias\_DampedPersistence\_h3} & \ph{skill\_vs\_persistence\_DampedPersistence\_h3} & \ph{skill\_vs\_climatology\_DampedPersistence\_h3} & \ph{n\_DampedPersistence\_h3} \\
DampedPersistence & 7 & \ph{RMSE\_DampedPersistence\_h7} & \ph{MAE\_DampedPersistence\_h7} & \ph{bias\_DampedPersistence\_h7} & \ph{skill\_vs\_persistence\_DampedPersistence\_h7} & \ph{skill\_vs\_climatology\_DampedPersistence\_h7} & \ph{n\_DampedPersistence\_h7} \\
Climatology & 1 & \ph{RMSE\_Climatology\_h1} & \ph{MAE\_Climatology\_h1} & \ph{bias\_Climatology\_h1} & \ph{skill\_vs\_persistence\_Climatology\_h1} & \ph{skill\_vs\_climatology\_Climatology\_h1} & \ph{n\_Climatology\_h1} \\
Climatology & 3 & \ph{RMSE\_Climatology\_h3} & \ph{MAE\_Climatology\_h3} & \ph{bias\_Climatology\_h3} & \ph{skill\_vs\_persistence\_Climatology\_h3} & \ph{skill\_vs\_climatology\_Climatology\_h3} & \ph{n\_Climatology\_h3} \\
Climatology & 7 & \ph{RMSE\_Climatology\_h7} & \ph{MAE\_Climatology\_h7} & \ph{bias\_Climatology\_h7} & \ph{skill\_vs\_persistence\_Climatology\_h7} & \ph{skill\_vs\_climatology\_Climatology\_h7} & \ph{n\_Climatology\_h7} \\
LightGBM & 1 & \ph{RMSE\_LightGBM\_h1} & \ph{MAE\_LightGBM\_h1} & \ph{bias\_LightGBM\_h1} & \ph{skill\_vs\_persistence\_LightGBM\_h1} & \ph{skill\_vs\_climatology\_LightGBM\_h1} & \ph{n\_LightGBM\_h1} \\
LightGBM & 3 & \ph{RMSE\_LightGBM\_h3} & \ph{MAE\_LightGBM\_h3} & \ph{bias\_LightGBM\_h3} & \ph{skill\_vs\_persistence\_LightGBM\_h3} & \ph{skill\_vs\_climatology\_LightGBM\_h3} & \ph{n\_LightGBM\_h3} \\
LightGBM & 7 & \ph{RMSE\_LightGBM\_h7} & \ph{MAE\_LightGBM\_h7} & \ph{bias\_LightGBM\_h7} & \ph{skill\_vs\_persistence\_LightGBM\_h7} & \ph{skill\_vs\_climatology\_LightGBM\_h7} & \ph{n\_LightGBM\_h7} \\
LSTM & 1 & \ph{RMSE\_LSTM\_h1} & \ph{MAE\_LSTM\_h1} & \ph{bias\_LSTM\_h1} & \ph{skill\_vs\_persistence\_LSTM\_h1} & \ph{skill\_vs\_climatology\_LSTM\_h1} & \ph{n\_LSTM\_h1} \\
LSTM & 3 & \ph{RMSE\_LSTM\_h3} & \ph{MAE\_LSTM\_h3} & \ph{bias\_LSTM\_h3} & \ph{skill\_vs\_persistence\_LSTM\_h3} & \ph{skill\_vs\_climatology\_LSTM\_h3} & \ph{n\_LSTM\_h3} \\
LSTM & 7 & \ph{RMSE\_LSTM\_h7} & \ph{MAE\_LSTM\_h7} & \ph{bias\_LSTM\_h7} & \ph{skill\_vs\_persistence\_LSTM\_h7} & \ph{skill\_vs\_climatology\_LSTM\_h7} & \ph{n\_LSTM\_h7} \\
ThermoRoute & 1 & \ph{RMSE\_ThermoRoute\_h1} & \ph{MAE\_ThermoRoute\_h1} & \ph{bias\_ThermoRoute\_h1} & \ph{skill\_vs\_persistence\_ThermoRoute\_h1} & \ph{skill\_vs\_climatology\_ThermoRoute\_h1} & \ph{n\_ThermoRoute\_h1} \\
ThermoRoute & 3 & \ph{RMSE\_ThermoRoute\_h3} & \ph{MAE\_ThermoRoute\_h3} & \ph{bias\_ThermoRoute\_h3} & \ph{skill\_vs\_persistence\_ThermoRoute\_h3} & \ph{skill\_vs\_climatology\_ThermoRoute\_h3} & \ph{n\_ThermoRoute\_h3} \\
ThermoRoute & 7 & \ph{RMSE\_ThermoRoute\_h7} & \ph{MAE\_ThermoRoute\_h7} & \ph{bias\_ThermoRoute\_h7} & \ph{skill\_vs\_persistence\_ThermoRoute\_h7} & \ph{skill\_vs\_climatology\_ThermoRoute\_h7} & \ph{n\_ThermoRoute\_h7} \\
\bottomrule\noalign{}
\end{tabular}
\end{sidewaystable}
...
\renewcommand{\arraystretch}{1.1}
\centering
\textbf{Table 4.8.} Held-out 2021--2023 station-median metrics, one-factor ablations.\\[3pt]
\begin{tabular}{@{}lrcccccc@{}}
\toprule\noalign{}
Model & $h$ (d) & RMSE ($^\circ$C) & MAE ($^\circ$C) & bias ($^\circ$C) & skill vs.\ persist & skill vs.\ clim & $n$ \\
\midrule\noalign{}
tr-fixedkappa & 1 & \ph{RMSE\_tr-fixedkappa\_h1} & \ph{MAE\_tr-fixedkappa\_h1} & \ph{bias\_tr-fixedkappa\_h1} & \ph{skill\_vs\_persistence\_tr-fixedkappa\_h1} & \ph{skill\_vs\_climatology\_tr-fixedkappa\_h1} & \ph{n\_tr-fixedkappa\_h1} \\
tr-fixedkappa & 3 & \ph{RMSE\_tr-fixedkappa\_h3} & \ph{MAE\_tr-fixedkappa\_h3} & \ph{bias\_tr-fixedkappa\_h3} & \ph{skill\_vs\_persistence\_tr-fixedkappa\_h3} & \ph{skill\_vs\_climatology\_tr-fixedkappa\_h3} & \ph{n\_tr-fixedkappa\_h3} \\
tr-fixedkappa & 7 & \ph{RMSE\_tr-fixedkappa\_h7} & \ph{MAE\_tr-fixedkappa\_h7} & \ph{bias\_tr-fixedkappa\_h7} & \ph{skill\_vs\_persistence\_tr-fixedkappa\_h7} & \ph{skill\_vs\_climatology\_tr-fixedkappa\_h7} & \ph{n\_tr-fixedkappa\_h7} \\
tr-nodynamicprior & 1 & \ph{RMSE\_tr-nodynamicprior\_h1} & \ph{MAE\_tr-nodynamicprior\_h1} & \ph{bias\_tr-nodynamicprior\_h1} & \ph{skill\_vs\_persistence\_tr-nodynamicprior\_h1} & \ph{skill\_vs\_climatology\_tr-nodynamicprior\_h1} & \ph{n\_tr-nodynamicprior\_h1} \\
tr-nodynamicprior & 3 & \ph{RMSE\_tr-nodynamicprior\_h3} & \ph{MAE\_tr-nodynamicprior\_h3} & \ph{bias\_tr-nodynamicprior\_h3} & \ph{skill\_vs\_persistence\_tr-nodynamicprior\_h3} & \ph{skill\_vs\_climatology\_tr-nodynamicprior\_h3} & \ph{n\_tr-nodynamicprior\_h3} \\
tr-nodynamicprior & 7 & \ph{RMSE\_tr-nodynamicprior\_h7} & \ph{MAE\_tr-nodynamicprior\_h7} & \ph{bias\_tr-nodynamicprior\_h7} & \ph{skill\_vs\_persistence\_tr-nodynamicprior\_h7} & \ph{skill\_vs\_climatology\_tr-nodynamicprior\_h7} & \ph{n\_tr-nodynamicprior\_h7} \\
tr-nomoe & 1 & \ph{RMSE\_tr-nomoe\_h1} & \ph{MAE\_tr-nomoe\_h1} & \ph{bias\_tr-nomoe\_h1} & \ph{skill\_vs\_persistence\_tr-nomoe\_h1} & \ph{skill\_vs\_climatology\_tr-nomoe\_h1} & \ph{n\_tr-nomoe\_h1} \\
tr-nomoe & 3 & \ph{RMSE\_tr-nomoe\_h3} & \ph{MAE\_tr-nomoe\_h3} & \ph{bias\_tr-nomoe\_h3} & \ph{skill\_vs\_persistence\_tr-nomoe\_h3} & \ph{skill\_vs\_climatology\_tr-nomoe\_h3} & \ph{n\_tr-nomoe\_h3} \\
tr-nomoe & 7 & \ph{RMSE\_tr-nomoe\_h7} & \ph{MAE\_tr-nomoe\_h7} & \ph{bias\_tr-nomoe\_h7} & \ph{skill\_vs\_persistence\_tr-nomoe\_h7} & \ph{skill\_vs\_climatology\_tr-nomoe\_h7} & \ph{n\_tr-nomoe\_h7} \\
tr-norouter & 1 & \ph{RMSE\_tr-norouter\_h1} & \ph{MAE\_tr-norouter\_h1} & \ph{bias\_tr-norouter\_h1} & \ph{skill\_vs\_persistence\_tr-norouter\_h1} & \ph{skill\_vs\_climatology\_tr-norouter\_h1} & \ph{n\_tr-norouter\_h1} \\
tr-norouter & 3 & \ph{RMSE\_tr-norouter\_h3} & \ph{MAE\_tr-norouter\_h3} & \ph{bias\_tr-norouter\_h3} & \ph{skill\_vs\_persistence\_tr-norouter\_h3} & \ph{skill\_vs\_climatology\_tr-norouter\_h3} & \ph{n\_tr-norouter\_h3} \\
tr-norouter & 7 & \ph{RMSE\_tr-norouter\_h7} & \ph{MAE\_tr-norouter\_h7} & \ph{bias\_tr-norouter\_h7} & \ph{skill\_vs\_persistence\_tr-norouter\_h7} & \ph{skill\_vs\_climatology\_tr-norouter\_h7} & \ph{n\_tr-norouter\_h7} \\
tr-notcn & 1 & \ph{RMSE\_tr-notcn\_h1} & \ph{MAE\_tr-notcn\_h1} & \ph{bias\_tr-notcn\_h1} & \ph{skill\_vs\_persistence\_tr-notcn\_h1} & \ph{skill\_vs\_climatology\_tr-notcn\_h1} & \ph{n\_tr-notcn\_h1} \\
tr-notcn & 3 & \ph{RMSE\_tr-notcn\_h3} & \ph{MAE\_tr-notcn\_h3} & \ph{bias\_tr-notcn\_h3} & \ph{skill\_vs\_persistence\_tr-notcn\_h3} & \ph{skill\_vs\_climatology\_tr-notcn\_h3} & \ph{n\_tr-notcn\_h3} \\
tr-notcn & 7 & \ph{RMSE\_tr-notcn\_h7} & \ph{MAE\_tr-notcn\_h7} & \ph{bias\_tr-notcn\_h7} & \ph{skill\_vs\_persistence\_tr-notcn\_h7} & \ph{skill\_vs\_climatology\_tr-notcn\_h7} & \ph{n\_tr-notcn\_h7} \\
tr-unbounded & 1 & \ph{RMSE\_tr-unbounded\_h1} & \ph{MAE\_tr-unbounded\_h1} & \ph{bias\_tr-unbounded\_h1} & \ph{skill\_vs\_persistence\_tr-unbounded\_h1} & \ph{skill\_vs\_climatology\_tr-unbounded\_h1} & \ph{n\_tr-unbounded\_h1} \\
tr-unbounded & 3 & \ph{RMSE\_tr-unbounded\_h3} & \ph{MAE\_tr-unbounded\_h3} & \ph{bias\_tr-unbounded\_h3} & \ph{skill\_vs\_persistence\_tr-unbounded\_h3} & \ph{skill\_vs\_climatology\_tr-unbounded\_h3} & \ph{n\_tr-unbounded\_h3} \\
tr-unbounded & 7 & \ph{RMSE\_tr-unbounded\_h7} & \ph{MAE\_tr-unbounded\_h7} & \ph{bias\_tr-unbounded\_h7} & \ph{skill\_vs\_persistence\_tr-unbounded\_h7} & \ph{skill\_vs\_climatology\_tr-unbounded\_h7} & \ph{n\_tr-unbounded\_h7} \\
\bottomrule\noalign{}
\end{tabular}
\end{sidewaystable}

\textbf{Table 4.9 --- interval and probability behaviour on the held-out keys.}
\ph{interval\_probability\_2021\_2023} --- by model and lead: empirical marginal
...
\textbf{Table 4.10 --- outcome quality control.} \ph{outcome\_qc\_2021\_2023} ---
immutable counts by station, variable, raw qualifier string, and value presence;
multiple-finite-series conflict counts; and the qualifier-restricted \texttt{\{A\}}
...
approximate sensitivities rather than decision evidence. No result
here is a superiority, non-inferiority, equivalence, or parity statement, and
none generalises to a national or U.S.-river population.
...
cells in Section 4.6 carry \ph{\dots} placeholders
(naming \texttt{model}, \texttt{horizon}, and \texttt{metric}, e.g.
\ph{RMSE\_ThermoRoute\_h1}) that are filled from
\texttt{outputs/\allowbreak{}conventional/\allowbreak{}holdout\_\allowbreak{}metrics\_\allowbreak{}2021\_\allowbreak{}2023.csv}; none of those is a
