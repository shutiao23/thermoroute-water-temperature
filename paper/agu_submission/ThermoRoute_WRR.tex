...
\textbf{Conditional coverage under non-exchangeability (ACI).} Split-CQR's formal
guarantee assumes exchangeability, so a natural worry is that the near-nominal
\emph{marginal} PICP hides \emph{conditional} under-coverage across regimes or regions. We
test this with Adaptive Conformal Inference (Gibbs \& Candès, 2021), which updates
the miscoverage level α\_t online along each station's 2019--2020 test sequence and
...
value is \emph{cross-region uniformity}: it collapses the per-HUC2 coverage spread from a
...
also score exceedance of the \textbf{fixed EPA 7-day-average-daily-maximum salmonid
...
(\texttt{scripts/01}--\texttt{20}, orchestrated in order by \texttt{scripts/run\_all.sh}: the 3-station
...
decision-value / probabilistic / TUURT analyses); \texttt{scripts/14\_manifest.py} writes a
