...
\item
  ...
  \emph{bounded} neural residual is guaranteed not to degrade the prior by more than
  ...
  §4.5 reports that this mechanism does \emph{not} generalise, and we retain the prior
...
global LightGBM. air2stream is the fair \emph{variant} of §3.5, calibrated on
...
wide margin; \textbf{between them the result is a horizon-conditional tie} --- LightGBM
is better at 1 day, the two are indistinguishable at 3 days, and ThermoRoute is
better at 7 days (the same ordering as in-sample, §4.2). We therefore do \textbf{not}
claim that ThermoRoute generalises to ungauged basins \emph{better than} the strong
learned baseline; we claim that a physics-biased, station-agnostic model transfers
\emph{as well as} a global GBDT while additionally carrying the calibrated intervals
...
ratios α: peak REV is \textbf{0.91 / 0.85 / 0.82} for ThermoRoute (1/3/7 d) versus
