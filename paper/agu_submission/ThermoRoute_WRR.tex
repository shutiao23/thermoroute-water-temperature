...
whole-HUC2 cluster level, Table 4.6). When the held station\textquotesingle s own thermal history remains available, whole-region
...
the reference-model effect --- and the learned model\textquotesingle s residual value
concentrates in rapid warming and rapid cooling states (−0.30 and −0.18 °C
station-median paired ΔRMSE at seven days); withholding the station\textquotesingle s thermal
...
histories from preprocessing, do not share the local cells\textquotesingle{} geometry ordering:
...
run reused fold 0\textquotesingle s statistics across folds (a recorded defect, DLOG-012), so
...
station\textquotesingle s own thermal history is unavailable: development-period three-day
...
withholding the station\textquotesingle s own thermal history from preprocessing (pooled
adaptation) costs roughly 0.2 °C at seven days --- about 40 times the geometry
effect at that lead --- and the pooled cells do not share the local cells\textquotesingle{}
geometry ordering (the 7-day region-minus-random penalty changes sign), so the
spatial partition is secondary specifically \emph{given} local thermal history
