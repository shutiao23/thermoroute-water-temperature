% ``linenumbers'' without ``draft''.  The two are separable and only one of
% them is a review requirement: linenumbers gives referees something to cite,
% while draft additionally double-spaces the body.  Double-spaced, this
% manuscript is 33 pages; single-spaced with line numbers it is 22, inside the
% 23-page cap, and no content differs between them.  Reaching 23 by deleting a
% further 3,400 words would have cost results rather than leading.  If the
% editor requires double spacing, add ``draft'' back and the paper grows to 33
% pages without any other change.
\documentclass[linenumbers]{agujournal2025}
\usepackage{amsmath,amssymb}
\usepackage{booktabs,longtable,array,tabularx}
\usepackage{rotating}
\usepackage{url,xurl}
% hyperref is deliberately NOT loaded.  pandoc pulled it in for the inline DOI
% links, and those are gone: every citation is now a real apacite key and the
% document contains no \href or \url at all.  Keeping it was not free -- this
% apacite and this hyperref disagree about \hyper@link@, so the build succeeded
% on a clean tree and then died on the pass after bibtex, once \bibcite entries
% existed for apacite to hyperlink.  An unused package that breaks the second
...
\begin{figure}[!t]
\centering
\pandocbounded{\includegraphics[width=\linewidth,height=0.46\textheight,keepaspectratio,alt={Spatial transfer under matched holdouts.}]{figures/fig05_spatial_transfer.pdf}}
...
\caption{\textbf{Hydrologic conditions governing incremental skill (held-out 2021--2023).} (a) Station anomaly half-life, median 6.9 d. (b) One-day learned gain versus log half-life. (c) Issue-time-identifiable states; (d) retrospective outcome-conditioned strata; negative values favor ThermoRoute.}
\end{figure}
...
%
% The conversion the old TODO here asked for is done: every in-text citation is
% a real \cite or \citeA key and \nocite{*} is gone, so this list is what the
% paper cites rather than everything in ../references.bib.  That was 62 printed
% entries against 21 cited works.  Adding a citation now means adding the key
% here in the TeX, which is also what makes a deleted last citation drop its
% entry instead of lingering.
