...
backwards in time. On the independent window the deep model\textquotesingle s
...
its benchmark. \href{https://doi.org/10.1080/02626668609491024}{Klemeš (1986)} set
...
\href{https://doi.org/10.1002/hyp.446}{Seibert (2001)} and
\href{https://doi.org/10.1002/hyp.6825}{Schaefli and Gupta (2007)} argued that
efficiency criteria are uninterpretable without an explicit benchmark, and
\href{https://doi.org/10.1002/hyp.11476}{Seibert et al. (2018)} formalized upper and
lower benchmarks as the pair a reported score must sit between.
\href{https://doi.org/10.5194/hess-13-1757-2009}{Andréassian et al. (2009)} proposed
standardized crash tests, \href{https://doi.org/10.1016/j.jhydrol.2015.01.024}{Pappenberger et al.
(2015)} asked how a forecaster
can know that a forecast is better,
\href{https://doi.org/10.5194/hess-23-4323-2019}{Knoben et al. (2019)} showed that a
familiar criterion carries an implicit benchmark that is not the one most
readers assume, and \href{https://doi.org/10.1175/JHM-D-17-0209.1}{Nearing et al.
(2018},
\href{https://doi.org/10.1029/2020WR028091}{2021)} placed benchmarking at the centre
...
(\href{https://doi.org/10.1016/j.jhydrol.2018.09.027}{Arsenault et al., 2018}).
...
(\href{https://doi.org/10.1080/02626667.2013.803183}{Hrachowitz et al., 2013};
\href{https://doi.org/10.1029/2019WR026065}{Kratzert et al., 2019}). None of these
...
(\href{https://doi.org/10.5194/hess-18-463-2014}{Gupta et al., 2014}); Section 3.6
...
\noindent
\begin{tabularx}{\dimexpr\linewidth-24pt\relax}{@{}lrrr>{\hsize=1.0000\hsize\raggedright\arraybackslash}X@{}}
\toprule\noalign{}
...
\caption{\textbf{Study sites and evaluation design.} (a) The 120 U.S. gauges
...
\caption{\textbf{The formulation every compared model shares.} (a) Issue-time
...
is a \emph{raw-target} model with site identity; ResidualLightGBM, the plain causal
...
\emph{Non-anticipating sequence encoding.} The strictly left-looking encoder of
...
\renewcommand{\arraystretch}{1.12}
\noindent
\begin{tabularx}{\dimexpr\linewidth-24pt\relax}{@{}l>{\hsize=1.0000\hsize\raggedright\arraybackslash}Xrl@{}}
\toprule\noalign{}
...
lead-specific decay fitted directly on lag-\emph{h} anomaly pairs instead of
\(\phi^h\); and a relaxed clip --- and rescored each against the \emph{published}
...
\texttt{DampedPersistence} station RMSEs to within 1e-8 °C, which is what makes the
...
(\texttt{outputs/\allowbreak{}final/\allowbreak{}anchor\_\allowbreak{}sensitivity.parquet}). Two ends of that range are
...
\(\phi^h\), produces a \emph{stronger} reference and cuts the reported gain to
...
specified. That is the paper\textquotesingle s own thesis applied to itself, and it sharpens
...
\caption{\textbf{Decomposition of reported skill on the held-out 2021--2023 window (116 reportable stations, common forecast-key registry).} (a) Station-median
...
(\texttt{ThermoRoute\ \allowbreak{}−\ \allowbreak{}LightGBM}, equation 2) are +0.046 °C at 1 day, +0.008 °C at 3
...
favored at 98\% of the 116 reportable stations --- the abstract\textquotesingle s one-day claim
...
window the deep model\textquotesingle s station-median skill is +0.250 against persistence
...
such: the smallest effect this cohort\textquotesingle s cluster structure can resolve at
α = 0.05 is larger than the effect observed, so the comparison is underpowered
rather than null (\texttt{outputs/\allowbreak{}final/\allowbreak{}family\_\allowbreak{}power.parquet}). The three
...
family\textquotesingle s power is not uniform and Table 4.6 should be read row by row. The
...
models). We therefore report the whole-region penalty as \emph{below this design\textquotesingle s
...
preprocessing reused fold 0\textquotesingle s statistics across folds 1--3, whose held-out
stations lie inside fold 0\textquotesingle s training set (a recorded defect, DLOG-012). We
...
\caption{\textbf{Spatial transfer under matched random-site and whole-region holdouts on the independent 2021--2023 window.} (a) The four whole-HUC2-region
...
are from \texttt{outputs/\allowbreak{}final/\allowbreak{}decomposition\_\allowbreak{}intervals.parquet} and carry the same
...
belong to the two \texttt{actual\_\allowbreak{}*} strata, and those are conditioned on the realized
...
\caption{\textbf{Hydrologic conditions governing incremental skill (held-out 2021--2023).} (a) Distribution of the station anomaly half-life from the
...
station\textquotesingle s own thermal history, leaving the transferable component worth about
...
local observation: \textbf{L0} keeps everything; \textbf{L1} pools the climatology and
damped rate over training stations, so the target site contributes no long-term
statistic; \textbf{L2} removes every target-site water-temperature input --- lags,
rolling statistics, deltas, their observedness flags, the climatology anomaly
and the persistence anchor --- leaving discharge and meteorology; \textbf{L2-U2}
removes discharge as well.

Prohibited inputs are dropped from the design matrix rather than filled, and
before each cell is fitted a proof perturbs the held stations\textquotesingle{} own observations
...
\textbf{Table 4.15 --- value of local thermal information.} Station-first paired
...
\renewcommand{\arraystretch}{1.12}
\noindent
\begin{tabularx}{\dimexpr\linewidth-24pt\relax}{@{}>{\hsize=1.0000\hsize\raggedright\arraybackslash}Xrrlr@{}}
\toprule\noalign{}
...
\textbf{All local thermal state (L2−L0)} & \textbf{1 d} & \textbf{1.716} & \textbf{{[}1.363, 2.442{]}} & \textbf{1.00} \\
& \textbf{3 d} & \textbf{1.255} & \textbf{{[}0.989, 2.064{]}} & \textbf{1.00} \\
& \textbf{7 d} & \textbf{1.325} & \textbf{{[}1.092, 1.725{]}} & \textbf{1.00} \\
...
the geometry penalty of Section 4.5. A study that keeps the target gauge\textquotesingle s
...
history.} Pooling a station\textquotesingle s climatology and damped rate over the training
...
\textbf{Discharge does not substitute for thermal history.} Once water temperature is
...
This is the interaction the transfer literature\textquotesingle s concern predicts, and it
...
tree models with the \emph{realized} meteorology over t+1 \ldots{} t+h substituted for the
...
difference is not cosmetic. F3 is a \textbf{retrospective realized gridded
...
operational gain. The analysis is also \emph{post-outcome}: the 2021--2023 window was
...
statistic; MDE is the smallest effect this cohort\textquotesingle s cluster structure can
...
\renewcommand{\arraystretch}{1.12}
\noindent
\begin{tabularx}{\dimexpr\linewidth-24pt\relax}{@{}>{\hsize=2.0841\hsize\raggedright\arraybackslash}Xr>{\hsize=0.7148\hsize\raggedleft\arraybackslash}X>{\hsize=0.7148\hsize\raggedleft\arraybackslash}Xr>{\hsize=0.7432\hsize\raggedright\arraybackslash}X>{\hsize=0.7431\hsize\raggedright\arraybackslash}Xrr@{}}
\toprule\noalign{}
...
It is \emph{large relative to what the design can resolve}: every value is three to
four times its own minimum detectable effect, where the architecture effects of
Section 4.3 are smaller than theirs. It is \emph{spatially stable}: the
...
region. And it is \emph{not a target-formulation artifact}: the station-level
...
\textbf{The gain is event-scale weather information, not seasonal phase.} A value
...
(\texttt{protocols/\allowbreak{}wrr\_\allowbreak{}forcing\_\allowbreak{}placebo\_\allowbreak{}protocol\_\allowbreak{}v5a\_\allowbreak{}seal.json}): the meteorology
vector is deranged across dates \emph{within each station-month}, so every station\textquotesingle s
...
\textbf{Table 4.14 --- shuffled-forcing placebo (sealed pre-outcome).} \(V_F\) is the
station-first forcing value in °C; \emph{retained} is the placebo\textquotesingle s share of the
...
\renewcommand{\arraystretch}{1.12}
\noindent
\begin{tabularx}{\dimexpr\linewidth-24pt\relax}{@{}>{\hsize=1.3036\hsize\raggedright\arraybackslash}Xrrrr>{\hsize=0.6964\hsize\raggedleft\arraybackslash}Xl@{}}
\toprule\noalign{}
...
the placebo is worth nothing at all: tomorrow\textquotesingle s weather is useful only as
tomorrow\textquotesingle s weather. By seven days about a twelfth of the value survives a
...
\textbf{What the forcing value is worth as a warning.} A station-median RMSE
...
(\texttt{outputs/\allowbreak{}final/\allowbreak{}forcing\_\allowbreak{}event\_\allowbreak{}metrics\_\allowbreak{}summary.parquet}).
...
station\textquotesingle s training 95th percentile rises from 0.18 to 0.55, and for rapid
warming from 0.18 to 0.52; the critical success index roughly triples in both
cases. Crucially this is not a base-rate trade: the false-alarm ratio \emph{falls}
...
\renewcommand{\arraystretch}{1.12}
\noindent
\begin{tabularx}{\dimexpr\linewidth-24pt\relax}{@{}>{\hsize=1.0000\hsize\raggedright\arraybackslash}Xllrr@{}}
\toprule\noalign{}
...
\textbf{Which rivers depend on future weather.} The station spread in the forcing
...
the anomaly half-life of the station\textquotesingle s own damped anchor, with Spearman
...
tomorrow\textquotesingle s weather less. The relationship is strongest at one day (ρ = −0.82)
...
(\texttt{outputs/\allowbreak{}final/\allowbreak{}forcing\_\allowbreak{}heterogeneity.parquet}).

\textbf{The model is using exact event timing.} The shuffle cannot separate
...
the record are dropped from both shift arms \emph{and} from the true arm, so all
three are scored on one common shift-registry named separately from the primary
one.

The forcing value does not survive a week\textquotesingle s displacement either. The +7-day arm
...
\textbf{The forcing value is future air temperature.} The last sealed arms attribute
...
\texttt{without} arm returns 0.62 °C, indistinguishable from the full value. The same
...
\textbf{About half the oracle survives a real forecast.} Everything above is an
...
reproduces the \texttt{only\_\allowbreak{}air\_\allowbreak{}temperature} arm exactly, with a single substitution:
...
model \emph{trained} on F2a features sees a future-temperature column that is
...
\textbf{Table 4.16 --- what an archived fixed-lead temperature forecast recovers.}
Station-first values in °C against F0 on 116 reportable stations; the
denominator is the realized-temperature oracle (\texttt{only\_\allowbreak{}air\_\allowbreak{}temperature}), never
...
\renewcommand{\arraystretch}{1.12}
\noindent
\begin{tabularx}{\dimexpr\linewidth-24pt\relax}{@{}>{\hsize=1.0000\hsize\raggedright\arraybackslash}Xrrrr@{}}
\toprule\noalign{}
...
series is a \textbf{fixed-lead composite} --- for each valid time, what a run issued
\emph{h} days earlier said about that day --- not a coherent trajectory from one
...
below are slightly smaller than Table 4.13\textquotesingle s; the comparison that matters is
...
at both, for both target formulations. Future weather is worth \emph{less} to a model
...
corrects is the persistence anchor built from the station\textquotesingle s own recent readings.
Remove that anchor and the model falls back on a pooled climatology, which is
too coarse a starting point for tomorrow\textquotesingle s weather to sharpen --- there is less
...
reason
(\texttt{outputs/\allowbreak{}final/\allowbreak{}forcing\_\allowbreak{}information\_\allowbreak{}interaction\_\allowbreak{}v1/\allowbreak{}}).
...
\href{https://doi.org/10.5194/hess-29-2521-2025}{Corona and Hogue (2025)} is a
...
(\href{https://doi.org/10.1080/02626668609491024}{Klemeš, 1986};
\href{https://doi.org/10.1002/hyp.6825}{Schaefli and Gupta, 2007};
\href{https://doi.org/10.5194/hess-22-4323-2018}{Seibert et al., 2018}) --- but the
...
held station\textquotesingle s own thermal history, the transferable part of the model is worth
...
\textbf{Cohort, measurement, and design.} The cohort trades a real, modest
...
station\textquotesingle s own thermal history, the model component that would have to transfer
...
\textbf{Reproduction.} Sections 4.1--4.3 are reproducible from the archived panel, the archived model
