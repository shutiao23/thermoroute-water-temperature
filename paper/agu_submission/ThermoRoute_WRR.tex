...
of 249,072 station/\allowbreak date/\allowbreak horizon keys at 120 U.S. gauges (116 reportable on the
...
seasonal adjustment of yesterday\textquotesingle s reading, most of the advantage that a naive
...
the long-form results table (columns \texttt{model}, \texttt{horizon}, \texttt{metric}, \texttt{value},
\texttt{n}). RMSE, MAE, bias, and skill are unweighted station medians over the
reportable stations on the common held-out forecast keys (Section 3.6), and
\texttt{n} is the reportable station count.
...
\renewcommand{\arraystretch}{1.12}
\noindent
\begin{tabularx}{\dimexpr\linewidth-24pt\relax}{@{}r>{\raggedright\arraybackslash}Xrrrrr>{\raggedleft\arraybackslash}X@{}}
\toprule\noalign{}
\# & Comparison & Lead & ΔRMSE (°C) & CI low & CI high & Win rate & p \\
\midrule\noalign{}
1 & ThermoRoute vs. damped persistence & 1 d & -0.129 & -0.199 & -0.090 & 0.90 & \textless0.001 \\
2 & ThermoRoute vs. damped persistence & 3 d & -0.108 & -0.140 & -0.079 & 0.91 & \textless0.001 \\
3 & ThermoRoute vs. damped persistence & 7 d & -0.069 & -0.088 & -0.057 & 0.95 & \textless0.001 \\
...
positive favors the candidate); and the reportable station count \emph{n} = 116.
...
ThermoRoute & 0.640 & 1.337 & 1.694 & 0.469 & 1.001 & 1.276 & \texttt{+0.010} & -0.028 & -0.096 \\
\bottomrule\noalign{}
\end{tabularx}
}

\textbf{Table 4.7b. Skill against persistence and damped persistence --- primary models.} Column groups are lead times in days. Skill is the unweighted median over reportable stations of the per-station
ratio 1 − RMSE\_model/RMSE\_reference (Section 3.6); positive favors the model.
...
Model & RMSE 1 & 3 & 7 & MAE 1 & 3 & 7 & bias 1 & 3 & 7 \\
\midrule\noalign{}
fixed κ & 0.635 & 1.333 & 1.695 & 0.467 & 0.996 & 1.282 & \texttt{+0.011} & -0.029 & -0.094 \\
no dyn. prior & 0.634 & 1.327 & 1.702 & 0.466 & 0.989 & 1.283 & +0.024 & -0.013 & -0.083 \\
no MoE & 0.643 & 1.345 & 1.698 & 0.470 & 1.006 & 1.282 & \texttt{+0.000} & -0.033 & -0.093 \\
...
unbounded & 0.634 & 1.333 & 1.695 & 0.467 & 0.997 & 1.282 & \texttt{+0.007} & -0.030 & -0.095 \\
\bottomrule\noalign{}
\end{tabularx}
}
...
\textbf{Table 4.9 --- interval and probability behavior on the held-out keys.}
...
fitted on the in-fold stations\textquotesingle{} 2006--2017 rows under two spatial partitions: four
...
\begin{figure}[!t]
\centering
\pandocbounded{\includegraphics[width=\linewidth,height=0.46\textheight,keepaspectratio,alt={Spatial transfer under matched spatial holdouts.}]{figures/fig05_spatial_transfer.pdf}}
\caption{Spatial transfer under matched spatial holdouts.}
\end{figure}
...
\emph{Station-agnostic LightGBM, unweighted station medians over the 118 scored
...
\textbf{Table 4.11 --- Station-median error-budget decomposition.} Unweighted medians
...
\emph{Memory and learned gains are unweighted medians over stations of the
...
\emph{Pooled paired RMSE difference over held-out keys within each state; negative
favors ThermoRoute.} The learned model\textquotesingle s incremental value is largest exactly
...
season and lead. The learned model\textquotesingle s incremental value concentrates in rapid
