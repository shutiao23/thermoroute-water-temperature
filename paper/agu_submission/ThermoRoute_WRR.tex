...
\textbf{Three evidence grades are used and they are not interchangeable.} Section 4.4
...
development keys (SI20). Sections 4.5--4.7 are \emph{post-outcome descriptive},
...
three and seven}, three to four times each cell\textquotesingle s minimum detectable effect. It
...
an archived fixed-lead composite. Crossed with L it is worth \emph{less} once local
...
the anchor built from the station\textquotesingle s own readings, so the two are complements.
