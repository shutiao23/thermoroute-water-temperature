...
`render_fig01_preopening_concept.py` deterministically renders the 139.7 × 186 mm,
three-panel Figure 1:

1. the retained-station map and nearest-neighbour geometry;
2. HUC2 cluster geometry, the frozen claim checks, and the resulting scope; and
3. the training-period persistence challenge at 1-, 3-, and 7-day horizons.
...
- `fig01_preopening_concept.png` — 600 dpi review raster;
...
Panel (a) maps the 120 retained stations with redundant HUC2 colour/marker
encoding, sizes markers by retained observed training-period `WTEMP` days, and
plots their nearest-neighbour distance distribution. Panel (c) uses only
exact-day observed `WTEMP` pairs with both endpoints in
...
Panel (b) counts HUC2 membership directly from the frozen station registry. The
...
`operational_replay_claim_allowed = false` fields. Panel (b) therefore binds and
...
vermilion `#F9E3D6`, and pale teal `#DCEFEA`. Panel (b) additionally enforces 2 mm
