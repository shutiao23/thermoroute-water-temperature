...
本合并报告由三位独立评审(MIT 环境工程教授、清华大学计算机系教授、IBM 高级研究员)在统一的对抗式审查协议下产出。协议流程为:**4 轮侦察**(通读论文、代码仓库、产物文件与 git 时间线)+ **6 视角对抗审查**(超参选择协议 / 统计口径与推断 / 数字一致性 / 代码正确性与可复现工程 / 物理机理正确性 / 基准公平性)+ **逐条对抗核查**(代码逐行、产物逐位重算、git 提交时间线比对,凡属专业推断处均标注"(推断)")+ **完整性补查**(对已收敛/被证伪的原始指控做反向核实,保留公允边界)。

三视角合计产出 **原始 58 条** 意见,跨视角去重后收敛为 **34 条独立缺陷**,再经逐条对抗核查确认 **50 条**(部分独立缺陷在不同 horizon / 产物 / 位置各自成立而拆分计数),其中 **critical 8 条 / major 27 条 / minor 15 条**。三份报告均独立给出 **Reject(允许完成全链重跑与证据链重建后重新投稿)** 的推荐意见。以下逐字保留三份报告全文,末尾给出综合意见。
...
**审稿人身份**: 环境工程教授, 研究方向为河流热动力学、水文数据协议与物理引导机器学习(MIT)。以下意见基于对论文、代码仓库与全部产物的逐条对抗式核查。
...
**m8. §4.1 "The only value ThermoRoute adds here is probabilistic ... the point baselines cannot provide"(L261-263)措辞回避了 LightGBM** — 仓库自己的 LightGBM 基线同样输出分位数与超温概率(`baselines.py:154-203`), 且没有有效证据表明 TR 的概率产品在级联上优于它(级联决策价值表已因 #17/#18 缺陷被撤回, 不能反向引用)。该句应改写为承认 LightGBM 亦提供概率预警、TR 概率产品在级联上无已证明的优势。
...
**审稿人身份**: 清华大学计算机系教授(机器学习方法 / 数据泄露 / 统计推断 / 数字一致性方向)
**审稿日期**: 2026-07-02
...
**C6. 级联(Track A)全部论文数字产自 git 历史之外、bug 修复之前的代码,HEAD 复现失败**
...
**M5. §4.4 将级联的 PICP 范围 0.65–0.91 冒充 40 站 pilot 的 n=36 逐站范围(实测 0.70–0.99)**
- 证据: L347-349 "(n=36 PICP range 0.65–0.91)";该数字在 L262 与 git 历史中均标注为三站级联;用 40 站 `usgs_predictions.parquet` 实算逐站 PICP 为 0.70–0.99(既欠覆盖也过覆盖)。经核查,挪用发生在提交 fb6c1bf(而非 6444fc0);"小样本逐站覆盖离散更大"的定性结论仍成立,但 0.65–0.91 无任何 40 站产物支撑。
- 修改要求: 以实算 0.70–0.99 替换或删除该定量对比;全文检索 "0.65–0.91" 确保级联与 pilot 两处各有产物支撑。
...
**m6. §4.1 "The only value ThermoRoute adds here is probabilistic" 回避了 LightGBM 在级联上同样输出分位数与超温概率**(L261-263;`baselines.py:154-203`)。已撤回的 decision_value 表不能反向坐实 LightGBM 更优,但该句应改写为承认 LightGBM 亦提供概率预警、TR 概率产品在级联上无已证明优势。
...
3. **单一真值链**: 一次 HEAD 完整重跑覆盖级联与 120 站全部表格,manifest 恢复 e531553 式全量 sha256 登记,每个 headline 数字可追溯到唯一输入文件。README、摘要、正文、Discussion 四处对同一声明给出同一数字、同一方向。
...
**审稿人视角**: IBM 高级研究员(代码正确性 / 可复现工程 / 基准规范方向)
...
**M11. §4.4 将级联的 PICP 范围 0.65–0.91 冒充 40 站 pilot 的逐站范围(实测 0.70–0.99)**
- 证据: L348 写 "n=36 PICP range 0.65–0.91"; 该数字在 git 历史(e2baf1a)中原本标注三站级联; 用 40 站预测文件实算逐站 PICP 为 0.729–0.989(pooled)。
- 危害: 用无产物支撑的数字论证"逐站紧致覆盖是大样本属性", 且方向误导(pilot 实为宽幅双向偏差而非单纯欠覆盖)。
- 修改要求: 用实算范围替换或删除该定量对比; 全文检索 "0.65–0.91" 确保级联与 pilot 各有产物支撑。
...
- **"seed mean" 实为预测集成**(B-C2 / C-C2): 清华与 IBM 两位独立复算,对等逐 seed 口径下 h7 优势崩塌(p=0.54),这是动摇"物理引导优于 LightGBM"核心叙事的关键发现。MIT 报告未独立展开此点,但在 C4 中提示"仓库中另有集成口径问题被其他审稿人指出",与之呼应。
- **air2stream 非 canonical**(A-C2 / C-C3): MIT 与 IBM 一致认定其为自造变体(缺 θ 加权结构、自造低温钳制),"beats canonical" 标签失实。IBM 补充 M2 指出该基线还被双重压制(插补标签校准 + 拒用当日观测气温)。
...
- **可复现工程链**(C 独有 C5/C6/M6-M10): IBM 从工程视角额外发现级联 Track A 用 HEAD 复现失败、原始 predictions.parquet 被覆盖销毁、manifest 退化为占位符、CI 常红、checkpoint 跨批静默复用、第三方论文扫描页入库。清华 C6 独立命中级联复现失败与代码状态不可考,与 IBM C5 高度一致;MIT 未涉及工程链(视角分工使然)。
- **物理机理图题矛盾**(A 独有 M4/m3/m5): MIT 从环境工程视角深入 κ-流量热记忆的图 4(a) 硬编码标题与撤回结论矛盾、k_flow 是全站单标量非逐站量、h1 物理解读方向错误——这是另两位未展开的领域纵深。
- **站点纳入审计脱节**(A 独有 M3): MIT 发现 site_id→USGS 站号映射错位,是 120→114 的真实成因,属水文数据协议层独有发现。
- **调参预算不对等**(B 独有 M3): 清华指出 TR 的 δ 经扫描而 LightGBM/Ridge 零搜索,在 +0.006 量级优势上不公平。
- **noPrior 消融被限幅混淆**(C 独有 M3): IBM 指出 ±1.5°C tanh 钳制使"先验贡献"被夸大。
...
3. **级联 Track A 用 HEAD 完整重跑 + 恢复证据链**。证据: `outputs/logs/run_experiments.log`(6/26)早于代码提交、`review_response.md:47-48`、manifest 占位符(提交 2179c15)、`predictions.parquet` sha256 灭失。重跑 01→08 重写 §4.1,恢复全量 sha256 manifest,预测产物受控存档。
...
- **§4.4 数字来源统一**: 给 `10_usgs_analysis.py:46-47` 加 v2 分支,重算 calibration/REV;PICP "0.65–0.91" 改实测(级联 0.800–0.913 / pilot 0.70–0.99)。
