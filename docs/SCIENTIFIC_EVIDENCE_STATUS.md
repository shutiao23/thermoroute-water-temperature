...
| DLOG-012–026 scientific chronology | `PROVISIONAL_NOT_YET_COMPLETE` | DLOG-012–014 and DLOG-016–026 record the observed decisions and corrections; DLOG-015 is explicitly labelled an after-outcome administrative reconstruction; DLOG-023 binds only label-free F2a acquisition, DLOG-024 only score-independent keys/context, DLOG-025 withdraws DLOG-018–022 results, and DLOG-026 freezes the adverse-lineage evidence | Commit the chronology and referenced artifacts together. Never cite DLOG-020–022 as current evidence, reconstructed DLOG-015 as prospective registration, DLOG-023 as an F2a result, DLOG-024 as arm promotion/model evidence, or DLOG-026 as a corrected model result |
...
| F2b as-issued coherent multi-variable forecast trajectory | `PLANNED` | No acquired vintage-preserving trajectory exists; Open-Meteo directs exact-run use to its Single Runs API, whose general archive does not cover the 2021–2023 GFS window | Acquire and freeze an exact initialization-time archive (or leave this arm planned); never relabel F2a as as-issued or compare its temperature-only value with the full F3 retrospective realized gridded meteorological oracle denominator |
...
| Validated physical/forced-hybrid reference | `PLANNED` | An explicitly unofficial air2stream-style sensitivity exists | Validate against official equations/reference cases or freeze a transparent equilibrium-response model before using process-comparator language |
...
## Gates closed or withdrawn through DLOG-026
