...
| Forcing-v5 observed-lineage point authority (F0/F3_full, two tree models) | `VERIFIED_AND_FROZEN` | `outputs/final/forcing_regime_v5_observed_point_authority_v1/` publishes 696 station-paired effects on 116 reportable stations per cell under corrected observed lineage. Status is `POINT_ESTIMATE_ONLY`; it creates no interval | Keep the point/inference split. This authority is post-outcome and descriptive; it may never be called confirmatory or prospectively registered, and F3 remains a retrospective oracle |
| Forcing-v5 inference authority | `VERIFIED_AND_FROZEN` | `outputs/final/forcing_regime_v5_observed_inference_authority_v1/` (DLOG-027) adds whole-HUC2 bootstrap intervals, exact 2^15 sign-flip tails, equal-HUC aggregates, leave-one-HUC2-out ranges, year/season strata recomputed from the shards, station-level lead and model double differences, and a minimum detectable effect. Forcing value 0.130/0.542/0.627 degC (LightGBM) and 0.125/0.578/0.605 (ResidualLightGBM) at 1/3/7 days; all intervals exclude zero and all LOCO ranges keep their sign | Placebo and component controls (`protocols/wrr_forcing_placebo_protocol_v5a.yaml`) are required before the value may be described as event-scale weather information rather than seasonal phase. No ratio against an architecture, local-information, or geometry effect is admissible until the crossed matrix exists |
| F3 shuffled-forcing placebo (arm F3_shuffle_month) | `VERIFIED_AND_FROZEN` | `outputs/final/forcing_placebo_v5a/` (30 shards) and `outputs/final/forcing_placebo_v5a_authority_v1/` (DLOG-028). Specification sealed at `protocols/wrr_forcing_placebo_protocol_v5a_seal.json` on a clean tree before any placebo outcome existed; decision rule P1 satisfied at every model and lead, with the placebo retaining 0.0/2.0/8.8% (LightGBM) and -0.5/1.7/8.4% (ResidualLightGBM) of the true forcing value at 1/3/7 days | The F0/F3 reference contrast it validates remains post-outcome and descriptive; sealing a control before its own outcome does not make the already-inspected result confirmatory. The arm removes within-month day-to-day correspondence only: it does not separate that from sub-monthly synoptic persistence (the +/-7-day shift arm) and attributes nothing to individual variables (the component arms). Both remain specified under the same seal and unrun |
| F3 retrospective realized gridded meteorological oracle | `PROVISIONAL_NOT_YET_COMPLETE` | Raw future F3 values/masks and exact evaluation keys/targets remain usable inputs, but every learned F0/F3 score, forcing value, HUC2 interval, LOCO summary, lead-order interpretation, and P-5 component from the v4 tree shards is withdrawn by DLOG-025. The old authorities prove only downstream arithmetic consistency | Rerun the viewed twelve-cell correction with preserved raw masks and labels, then issue new point/inference authorities before quoting any forcing number. F3 must still be called a retrospective gridded oracle, never an operational gain. The shuffled placebo (DLOG-028) establishes the value is event-scale rather than seasonal-phase information; timing specificity and variable attribution remain open |
