...
| 7.10 | Vendored 2025 class assets resolved for release and rights | DONE (archive) / **RIGHTS** (repo exposure) | 2026-08-05, R10. Per-byte review recorded in `docs/RIGHTS_PROVIDER_EVIDENCE_20260801.md` §6: all five are `EXCLUDE_PUBLIC`, applying the §5 disposition that already covered "AGU 2025 class/macros/logos". Resolution: the archive **excludes** them and `paper/agu_submission/README.md` — itself an archive member — gives the upstream source (`AGU-Publications/agujournal2025-latex-template`) and each SHA-256, so the build stays reproducible. `verify_release.py` rejects them by name via `EXCLUDED_THIRD_PARTY_CLASS_ASSETS` and requires the README instructions whenever the archived `.tex` loads `agujournal2025`. No check was relaxed; `ALLOWED_PAPER_MEMBERS` is unchanged. Still open: `agujournal2019.cls` is a dead-weight archive member, and repository/history exposure of the five files. Full record: `docs/R10_AGU2025_RELEASE_ASSETS.md`. |
