...
| 6.4 | Convert in-text linked author-year prose to `\cite`/`\citeA` keys and emit a reference list | **AUTHORS** | The generated TeX currently produces **no reference list**. This is a citation-convention decision; once taken, `build_agu.py` must emit `\bibliography{../references}` (class default style `apacite`). |
...
| 7.3 | `paper/agu_submission/ThermoRoute_WRR.tex` regenerated | **BUILD** + **PROTOCOL** | `pandoc`/`pypandoc` is absent here, and the PRE-OPEN render guard refuses while the frozen source hashes do not match. The checked-in TeX still carries the **superseded** structure and is not submittable. |
| 7.4 | `build_agu.py --check` passes | **BUILD** | Same two blockers |
| 7.5 | PDF compiles cleanly through two `pdflatex` passes | **BUILD** | Requires 7.3 first |
...
| 8.4 | `preopen_document_sha256` for `paper/agu_submission/ThermoRoute_WRR.tex` | **PROTOCOL** | Hash still matches, but the bytes are the superseded manuscript. Re-seal only after 7.3 regenerates it. |
...
6. Install `pandoc`, re-seal the four frozen documents (§8), regenerate the TeX,
   and compile.
7. Convert the citation convention and generate the reference list (6.4).
