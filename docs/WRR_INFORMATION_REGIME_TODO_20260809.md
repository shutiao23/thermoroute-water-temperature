...
| T13a | P0 | F3 within-month shuffled placebo | **COMPLETE: DLOG-028**, sealed pre-outcome, P1 satisfied at every model and lead (retention 0.0-8.8%) | Done. First result in the project whose interpretation rule was frozen before its outcome |
| T13b | P0 | F3 +/-7-day shift and forcing components | SPEC FROZEN, UNRUN: `protocols/wrr_forcing_placebo_protocol_v5a.yaml` | Seal the v5a spec BEFORE the first placebo fit -- this is the project's only remaining chance at a pre-outcome forcing result. Then run true, within-month shuffled and +/-7-day shifted arms plus predeclared single and leave-one-out components on matched keys, and apply decision rules P1-P4 |
...
| T20 | P1 | Inference and interactions | FORCING SLICE COMPLETE: `outputs/final/forcing_regime_v5_observed_inference_authority_v1` (DLOG-027); L/G/A slices BLOCKED BY T11 | Station-first effects, two-stage seed aggregation, 10,000-draw HUC2 bootstrap, sign flips, LOCO, Holm family and F-by-L/G/A interactions |
| T21 | P1 | Anchor sensitivity, decomposition intervals and MDE | MDE DONE for the forcing cells (DLOG-027); anchor variants and decomposition intervals STILL OPEN | Predeclared anchor variants, cluster intervals and resolvable-effect calculation are frozen and reported |
...
| T24 | P0 | AGU build and length closure | PARTIAL (DLOG-027): abstract 248 words; figures renumbered 1-4 and double-captioning removed at the builder; equation refs (1)-(3) all defined; Section 6.x and 4.7 dangling refs fixed; md/TeX synchronized and PDF rebuilt. REMAINING: tables still numbered 4.6/4.7a/4.12/4.13 instead of 1..n, and the PDF is 47 pp against the ~25 PU guidance | Tables renumbered continuously across manuscript, ledger span tokens, table generator and TeX; length reduced |
...
## DLOG-027 pass (2026-08-10) — what changed

Closed this pass:

- **T20 (forcing slice).** The reserved inference path is now populated:
  `scripts/final/build_forcing_v5_inference_authority.py` publishes whole-HUC2
  bootstrap intervals, exact 2^15 sign-flip tails, equal-HUC aggregates,
  leave-one-HUC2-out ranges, year and season strata recomputed from the shards,
  station-level lead and model double differences, and a minimum detectable
  effect.  Nineteen tests cover it.  The builder independently recomputes every
  station effect from the twelve shards and fails closed on any drift from the
  published point authority.
- **Phase-0 quarantine, made mechanical.**  Claim statuses
  (`PRIMARY_FROZEN` / `DESCRIPTIVE_PROVISIONAL` / `NOT_USED`) plus three new
  gates and six injection tests.  The gate caught a live defect on first run:
  the outcome-conditioned -0.300 degC value was an Abstract sentence and a Key
  Point.
- **Manuscript scope.**  Removed the outcome-conditioned headline, the
  defective pooled +0.20 degC adaptation effect, and the "small confirmed
  geometry effect" reading.  Added Section 4.7 for the forcing result, labelled
  post-outcome and oracle-bounded and deliberately kept out of the Abstract and
  Key Points.  Rewrote the Introduction and Discussion 5.1 against eighteen
  newly added benchmarking and comparative-hydrology references.
- **T24 (partially).**  See the row above.

Deliberately *not* done this pass, with reasons:

- **The information-regime rewrite (T22).**  The advisor's own rule is not to
  rewrite the final Abstract before the 48-cell matrix exists; doing it now
  would change the paper's central claim a third time.  The Abstract was
  corrected to be defect-free and correctly scoped, not reframed.
- **Running the placebos (T13).**  Writing and running them in the same pass
  would make them post-outcome, which is exactly what the project cannot
  afford again.  The full specification is frozen in
  `protocols/wrr_forcing_placebo_protocol_v5a.yaml` with decision rules P1-P4
  fixed in advance, including P3, under which the forcing finding is withdrawn.
  Seal it, then run it.
- **Table renumbering.**  It touches the manuscript, the ledger span tokens,
  the table generator and the TeX simultaneously; it is cosmetic and was not
  worth risking the gate in the same pass as the scientific corrections.
