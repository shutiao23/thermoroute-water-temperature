...
asserts that every comparable model emits predictions on the SAME canonical
(site_id, horizon, issue_date, target_date) keys on the test split. Without this guarantee,
...
from thermoroute.registry import FORECAST_KEY, ROUTE_A_PRIMARY_MODELS

COMPARABLE_MODELS = ROUTE_A_PRIMARY_MODELS
...
def test_test_split_keys_identical_across_models(path):
    ...
    df = pd.read_parquet(path, columns=["model", "split", *FORECAST_KEY])
    df = df[df["split"] == "test"]
    present = tuple(model for model in COMPARABLE_MODELS if model in set(df["model"]))
    if len(present) < 2:
        pytest.skip(f"only {present} present in {path.name}; need ≥2 for comparison")
    df["issue_date"] = pd.to_datetime(df["issue_date"])
    df["target_date"] = pd.to_datetime(df["target_date"])
    df["key"] = list(df.loc[:, FORECAST_KEY].itertuples(index=False, name=None))
